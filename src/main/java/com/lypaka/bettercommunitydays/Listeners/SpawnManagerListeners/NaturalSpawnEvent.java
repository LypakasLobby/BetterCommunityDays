...
public class NaturalSpawnEvent {

    @SubscribeEvent
    public void onNaturalSpawn (AreaNaturalSpawnEvent event) {

        ServerPlayerEntity player = event.getPlayer();
        List<Pokemon> toSpawn = event.getPokemonSpawned();
        ...
        for (Map.Entry<String, String> entry : day.getSpawnManagerAreas().entrySet()) {
            ...
            if (entry.getKey().equalsIgnoreCase(area.getName())) {
                ...
                if (spawners.contains("natural")) {

                    List<Pokemon> updated = new ArrayList<>(toSpawn.size());
                    for (int i = 0; i < toSpawn.size(); i++) {

                        Pokemon communityDaySpawn = CommunityDayHandler.buildCommunityDaySpawn(day);
                        Pokemon originalSpawn = toSpawn.get(i);
                        CommunityDaySpawnEvent cdse = new CommunityDaySpawnEvent(player, communityDaySpawn, originalSpawn);
                        ...
                        if (!cdse.isCanceled()) {
                            ...
                            updated.add(i, communityDaySpawn);

                        } else {

                            updated.add(i, originalSpawn);

                        }

                    }

                    event.setPokemonSpawned(updated);
                    break;
