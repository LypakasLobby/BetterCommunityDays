...
public class RockSmashSpawnEvent {

    @SubscribeEvent
    public void onRockSmashSpawn (AreaRockSmashSpawnEvent event) {

        ServerPlayerEntity player = event.getPlayer();
        Pokemon pokemon = event.getToSpawn();
        ...
        for (Map.Entry<String, String> entry : day.getSpawnManagerAreas().entrySet()) {
            ...
            if (entry.getKey().equalsIgnoreCase(area.getName())) {
                ...
                if (spawners.contains("rocksmash") || spawners.contains("rock smash")) {
